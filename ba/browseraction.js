...
function checkTabWiki(tabData) {
	if(!tabData.wiki) // if no wiki
		...
		for(const [k, v] of Object.entries(tabData.categories)) {
			html += `<p name="currentCategories" data-name="${k}" data-href="${v}" title="${browser.i18n.getMessage("titleSave")}"><i class="icon"><img src="../heroicons/folder-arrow-down.svg"></i> ${k}</p>`;
		}
